...
export class CellGroups {
  ...
  cellToGroup: Record<number, Record<number, GroupID>>; // cellToGroup[y][x]
  constructor(groupsCells: Map<GroupID, Array<Coords>>) {
    const self = this;
    self.groupToCells = groupsCells;
    const cellToGroup: Record<number, Record<number, GroupID>> = {};
    groupsCells.forEach(function (cells, groupID) {
      cells.forEach(function (coords) {
        if (!(coords.y in cellToGroup)) {
          cellToGroup[coords.y] = {};
        }
        const row = cellToGroup[coords.y];
        row[coords.x] = groupID;
      });
    });

    self.cellToGroup = cellToGroup;
  }
}
